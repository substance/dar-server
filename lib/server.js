...
*/
app.put('/:dar', multer.fields([]), (req, res) => {
  let id = req.params.dar || 'default'
  let archiveDir = path.join(rootDir, id)
  fs.stat(archiveDir, async (err) => {
    if (err) return res.status(404)
    ...
    try {
      let archive = JSON.parse(formData._archive)
      let version = await readVersion(archiveDir)
      ...
      // binaries are in separate parts, so we pull them into the records
      Object.keys(archive).forEach(key => {
        let record = archive[key]
        if (record.encoding === 'blob') {
          record.data = formData[record.id]
        }
      })
